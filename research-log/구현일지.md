...
# 데이터 분할 및 각 GPU로 전송
device0_data = data[:500].to('cuda:0')
device1_data = data[500:].to('cuda:1')
...
# 비동기 연산 정의
with torch.cuda.stream(stream0):  # 첫 번째 GPU에서 비동기로 처리
    output0 = device0_data.mean(dim=0)

with torch.cuda.stream(stream1):  # 두 번째 GPU에서 비동기로 처리
    output1 = device1_data.mean(dim=0)

# 두 GPU의 연산이 끝나기를 기다림
torch.cuda.synchronize()  # 모든 GPU 연산 동기화

# 결과 병합
final_output = (output0.to('cpu') + output1.to('cpu')) / 2
print(final_output)
  ```
